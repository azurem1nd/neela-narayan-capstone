...
description: Join session boundaries with per-track memory-category classification so a session inherits a context label from its qualifying member tracks (plurality vote, with a minimum-representation floor), with a session-native fallback (Glimpse/Block/Exploration) whenever no label meets that floor. Turns raw sessions + track labels into the "contexts" shown to a user and used for playlist creation.
---
...
def _fallback_context(session, distinct_track_count):
...
- [ ] A session where the plurality winner covers less than 15% of distinct tracks falls through to the session-native fallback, not the thin-signal label
- [ ] A session with zero qualifying tracks and 1 distinct track labels as `Glimpse`
- [ ] A session with zero/insufficient qualifying signal, >1 distinct track, and a low artist-diversity ratio labels as `Block`
- [ ] A session with zero/insufficient qualifying signal, >1 distinct track, and a high artist-diversity ratio labels as `Exploration`
- [ ] A fallback context's description track count matches its own `track_count` field exactly (regression check for the fixed play-event/distinct-track mismatch)
